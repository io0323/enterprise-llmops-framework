...
Phase 1 の N-026(mock Fallback が評価スコア 84.29 を捏造した件)を前提にしてください。
これは Phase 2 では致命傷になります。評価系では次を必ず守ること:

- judge 論理モデルに fallback_to を設定しない。Judge が落ちたら
  そのケースを error として記録し、run を止めずに「スコア欠損」として扱う。
  代替モデルで穴埋めしない(誰が採点したか分からないスコアは使えない)
- eval 実行時に degraded=1 の span が混ざったら、その run の verdict を
  無条件で fail にする。縮退実行の結果で品質を判定しない
- mock Adapter を評価の実行経路に入れられないようにする。
  --model mock-echo での eval run は「配線確認モード」として明示し、
  その run は eval_runs に baseline として記録しない
- extract_json は Prompt 内の出力例 JSON を拾いうる(N-026 の直接原因)。
  Judge の応答パースでは、期待スキーマに合わない JSON を
  「パース成功」と扱わないこと。スキーマ検証まで通って初めて成功とする

N-026 は「評価基盤が偽の合格を作れる」という種類のバグです。
Phase 2 の全設計判断で、迷ったら「黙って通す」より「止める・欠損として残す」を選んでください。
```
