...
**盲点: 持続した異常は検知から消える(月次で補う)。**
`baseline_days: 7` の移動中央値なので、異常な水準が4日も続けばそれが中央値になり、
以後この検知は沈黙する。スケジューラの二重起動のような持続型は、レポートを回す
周期次第で**一度も警告が出ないまま常態化しうる**。

これは移動基準の性質であって、検知側で直すべきではない。閾値を厳しくすれば
誤検知が増え、上で書いたとおりレポート自体が読まれなくなる。基準を固定値にすれば、
それは結局ハード上限と同じ運用になる(N-045 の逆転をもう一度作る)。

そこで**月次に絶対値の比較**を置いた。`llmops report cost --compare-previous` が
`--since` と同じ長さの前期間を自動で取り、system 別に実課金・予算対象外・trace 総数を
**実数で**並べる。倍率を使わないのは、前期間が 0 のときに倍率が意味を持たないのと、
水準の変化は差のほうが読みやすいため。

役割分担を `docs/04` §4 に明記した:

- **週次**(移動中央値に対する倍率)= 突然の**跳ね**を見る。持続型は拾えない
- **月次**(前期間との実数差)= ずれたままの**水準**を見る。一時的な跳ねは均される

週次が沈黙しても月次で気付ける、という二段構えにしている。どちらも警告のみで、
止める判断は人がする点は変わらない。

### N-047 週次レポートの定期実行に launchd を選び、「動いていないこと」を可視化した
Step 3-7 は「既存システムのスケジューラから `llmops report weekly` を叩く」形を
想定していたが、**launchd(ユーザ LaunchAgent)にした**。

既存スケジューラ(DDE / CGMP の `scheduler/jobs.py`)に足す案を採らなかった理由:

- 5システムのコードを移行手順(docs/05)の記載外で変更しないのが絶対ルール1。
  レポート実行は docs/05 のどの手順にも含まれていない
- DDE/CGMP は長期テスト中で、ジョブを1つ足せば回帰の可能性を持ち込む。
  **ELF の都合で被管理システムを触るのは、Control Plane の立場として逆**
- ELF のレポートが DDE のスケジューラに依存すると、DDE を止めた期間だけ
  ELF の観測も止まる。監視される側に監視の起動を委ねない

ELF 自身を常駐させない点は Step 3-7 / docs/02 §9 のとおり。launchd が週に1度
CLI を1回呼ぶだけで、プロセスは増えない。

**設定の実体はリポジトリに置いた**(`ops/launchd/io.elf.weekly-report.plist` と
`ops/weekly_report.sh`)。マシン側の `~/Library/LaunchAgents` にしか無い状態にすると、
「なぜか毎週ファイルが増える」が説明できなくなり、環境を作り直したときに消える。
テストで plist の存在と妥当性(`plutil -lint`)を見ている。

**本題は定期実行ではなく、未実行の可視化のほう。**
N-046 で「検知しても止めない」を選んだ以上、警告が人に届くことが唯一の防御になる。
その経路が「手で叩く」だけだと、二層(週次の跳ね / 月次の水準)の検知が丸ごと
「人が思い出すこと」という1点に乗る。**動かしていない期間があっても気付けない**のが
本当の弱点なので、そこを塞いだ:

- 週次レポートを生成したら `audit_logs` に `report.weekly` を残す
- 前回生成から `report.weekly_stale_days`(既定10日)を超えていたら、
  **`llmops` のどのコマンドでも** 1行 stderr に出す
- **出すだけ。** 止めないし、レポートを自動実行もしない(勝手に走らせると、
  「気付かせる」ためのはずの仕組みが、気付かないまま動く仕組みになる)
- 既定を7日ではなく10日にしたのは、週次の実行が数日ずれただけで鳴らないため。
  鳴りっぱなしにしない方針は N-046 と同じ
- DB が無い(`llmops init` 前)・設定が読めない場合は黙る。
  通知のためにコマンドを止めない(絶対ルール4と同じ扱い)

launchd が落ちても、次に誰かが `llmops` を叩いた時点で「23日経過」が出る。
定期実行と可視化は別々に壊れるので、両方を入れて初めて成立する。
